...
import KitchenDashboardContent from "../../dashboard/page"

export default function KitchenDashboardWithRestaurant() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    if (user && user.restaurantId && user.restaurantId !== restaurantId) {
      ...
      router.push(`/kitchen/${user.restaurantId}/dashboard`)
    }
