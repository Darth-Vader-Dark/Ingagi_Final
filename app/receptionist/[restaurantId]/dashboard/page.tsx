...
import ReceptionistDashboardContent from "../../dashboard/page"

export default function ReceptionistDashboardWithRestaurant() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    if (user && user.restaurantId && user.restaurantId !== restaurantId) {
      ...
      router.push(`/receptionist/${user.restaurantId}/dashboard`)
    }
